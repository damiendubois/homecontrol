...
var programService = require('../modules/program/services/program-service');

User.find({ name: 'admin' }, {}, function(err, users) {
    if (users.length === 0) {
        User.create({ name: 'admin', permissions: ["admin"], password: "password", email: "dubois.dam@gmail.com" },
            function() {
...
programService.scheduleAllPrograms();